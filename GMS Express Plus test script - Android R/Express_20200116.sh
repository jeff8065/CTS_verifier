...
function serialAndToolToArray(){
	...
	brand=$(adb -s ${serialArray[$1]} -s ${serialArray[$1]}  -s ${serialArray[$1]} shell getprop ro.product.brand | sed 's/\r//' )
	name=$( adb -s ${serialArray[$1]} -s ${serialArray[$1]}  -s ${serialArray[$1]} shell getprop ro.product.name | sed 's/\r//' )	
	today=$(date +"%Y%m%d")
	name="go_check_""$name"'_'"$today"
	meminfo_text=$(adb -s ${serialArray[$1]} shell dumpsys meminfo)
